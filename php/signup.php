...
validazione($username, "Inserire uno username");
...
if ($birthdate != "" && $san->validateDate($birthdate) && $birthdate > "1900-01-01" && $birthdate <= date("Y-m-d")) {
    ...
    echo (str_replace("{{messaggioerrore}}", "<p class=\"error_message\"><em>Inserire una data valida</em></p>", $ret));
    die();
}
...
if ($connection->insertNewUser($username, password_hash($password, PASSWORD_BCRYPT), $name, $lastname, "", "", "", "", "") != false) {
    ...
    <p class=\"error_message\"><em>Username non disponibile, prova a cambiarlo<em></p>
    ", $ret));
    die();
}
